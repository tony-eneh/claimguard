...
\begin{abstract}
Auto-insurance platforms rely on cloud infrastructure to store and process diverse evidence assets, including first notice of loss (FNOL) reports, medical documents, telematics logs, photographs, and high-resolution accident videos. Although recent blockchain approaches provide tamper resistance and partial access control, but they lack a unified, enforceable model capable of governing heterogeneous resources, supporting action-level permissions, and binding authorization decisions to off-chain storage. This paper introduces \textbf{ClaimGuard}, a PureChain-backed, verifiable, attribute-based access control (ABAC) architecture designed for multi-stakeholder auto-insurance evidence workflows. ClaimGuard evaluates subject, object, action, and
contextual attributes on-chain and issues short-lived capability tokens to securely mediate access to cloud/IPFS evidence while ensuring auditability, revocation, and resilience against honest-but-curious cloud providers. A comprehensive evaluation compares ClaimGuard with a baseline cloud-only role-based access control (RBAC) model across varying load, policy-update frequency, policy churn under scale, and adversarial scenarios, both on local networks (Hardhat) and public testnets (Sepolia). Results show that ClaimGuard introduces modest latency overhead on local networks (60.9\,ms P50 vs.\ 1.9\,ms for pure RBAC) while significantly improving authorization correctness, policy integrity, and cross-organizational trust. Policy evaluation latency remains stable (2--3\,ms median) from 11 to 1{,}000 active rules, and revocation propagates 
within 1.1\,ms. Public testnet evaluation validates feasibility on real blockchain infrastructure. The system fills a key gap identified in the literature by providing the first resource-agnostic, action-aware, enforceable access control framework tailored to the operational requirements of modern auto-insurance evidence management.
\end{abstract}
...
Existing blockchain-based approaches for insurance data sharing either couple authorization to cryptographic schemes (e.g., ciphertext-policy attribute-based encryption or proxy re-encryption) that do not generalize cleanly to multi-actor, multi-format evidence workflows~\cite{sun2020insurance, liu2021auto}, or enforce coarse RBAC rules within smart contracts without addressing off-chain storage enforcement~\cite{raikwar2018insurance, tekale2023claims}. No prior work provides a unified, resource-agnostic, action-aware access control model that binds verifiable on-chain policy decisions to off-chain cloud or IPFS storage via enforceable mechanisms.

This paper introduces \textbf{ClaimGuard}, a PureChain-backed attribute-based access control (ABAC) architecture that evaluates \emph{(subject, object, action, context)} tuples on-chain and enforces decisions off-chain via a policy enforcement gateway issuing short-lived capability tokens. ClaimGuard decouples authorization from encryption, enabling transparent policy evaluation for heterogeneous evidence types (structured documents, media files, sensor logs) while maintaining compatibility with existing cloud storage backends. The system supports immediate revocation, time-bounded access, case-specific scoping, and immutable audit trails, directly addressing the limitations of both traditional RBAC and prior blockchain approaches.
...
Together, these contributions position ClaimGuard as the first end-to-end, verifiable, and enforceable access control system tailored for the operational realities of auto-insurance evidence management.
...
This section defines the auto-insurance evidence setting, presents the threat model, outlines functional and security requirements, and describes the ClaimGuard architecture and enforcement mechanisms.

\subsection{Problem Statement and System Context}

\subsubsection{Stakeholders and Evidence Pipeline}

The auto-insurance evidence workflow involves a diverse set of stakeholders with varying access rights and contextual constraints: policyholders (evidence owners), primary insurers (case managers and adjusters), reinsurers (financial auditors), repair workshops and assessors (damage evaluators), police and courts (legal investigators), regulators and auditors (compliance reviewers), and untrusted cloud storage providers (infrastructure operators). Evidence flows through multiple stages: first notice of loss (FNOL), evidence collection (documents, media, telematics), assessment, investigation and dispute resolution, settlement, and long-term auditing. Evidence assets include structured documents (FNOL reports, invoices, repair estimates), unstructured media (photographs, high-resolution accident videos), sensor logs (telematics, GPS traces), and derived artifacts (fraud signals, expert assessments, claim summaries).

Evidence is stored in cloud object stores (e.g., S3, Azure Blob) or IPFS for scalability and cost efficiency, while a consortium blockchain maintains shared policy state and immutable audit trails. The core challenge is enforcing blockchain-backed authorization decisions over off-chain storage backends that do not natively inherit on-chain access semantics. Stakeholders require action-level permissions---\emph{read, append, update, delete, adjudicate, disclose}---often constrained by case scope, time windows, jurisdictional mandates, and legal purpose (e.g., fraud analysis vs. litigation discovery).

\subsubsection{Representative Access Scenarios}

Representative scenarios motivating verifiable attribute-based access control include:
\begin{itemize}
    \item \textbf{Claim assessment:} Insurers and adjusters read and append case evidence; repair shops receive scoped read-only access to damage photos and estimates but cannot view medical records or modify adjuster notes.
    \item \textbf{Legal investigation:} Police and courts receive time-bounded, purpose-limited access with immediate revocation upon case closure, preventing continued access to sensitive evidence after legal mandates expire.
    \item \textbf{Multi-insurer coordination:} Reinsurers access specific claim subsets for financial audits while excluding unrelated or overly sensitive evidence such as personally identifiable health information.
    \item \textbf{Fraud detection:} Fraud investigators access assigned cases with strong auditability, ensuring that cross-case access attempts are logged and denied.
\end{itemize}
...
ClaimGuard anchors authorization decisions on PureChain smart contracts and enforces access to off-chain evidence via a Policy Enforcement Gateway (PEG). The contracts act as a tamper-resistant policy decision point (PDP), while the file server checks for validity of capability token before retrieval of insurance evidence from cloud/IPFS backends (Fig.~\ref{fig:architecture}).
...
%             minimum width=2.25cm, minimum height=0.9cm,
%             text width=2.25cm,
%             align=center,
%             inner sep=3pt
%         },
...
%     \node[box, below left=0.8cm and 0.6cm of policy] (subject) {Subject\\Registry};
%     \node[box, below right=0.8cm and 0.6cm of policy] (evidence) {Evidence\\Registry};
...
%     \end{tikzpicture}
%     \caption{ABAC evaluation flow: PEG calls \texttt{checkAccess}; contracts evaluate attributes and metadata; PEG returns a capability.}
...
\subsubsection{Subject Attribute Registry}
...
\subsubsection{Evidence Registry}
...
\subsubsection{Access Policy Manager}
...
\subsubsection{Capability-Based Access and Cloud Insider Resistance (R3)}
...
\paragraph{Optional Envelope Encryption}
...
\paragraph{Insider Attack Mitigation}
...
\subsubsection{Policy Enforcement Gateway}
...
The PEG is a stateless policy enforcement point that authenticates clients (e.g., OAuth or JWT), maps identities to blockchain addresses, queries \texttt{checkAccess} on-chain (with subject, resource, and action parameters), and issues short-lived capabilities (e.g., presigned URLs) only when allowed. It does not re-implement authorization logic locally; it simply enforces the contract decision and limits access duration.
...
This paper presented ClaimGuard, a blockchain-based attribute-based access control architecture that addresses the critical challenge of governing multi-stakeholder access to sensitive auto-insurance evidence. Traditional cloud-centric RBAC solutions fail to provide fine-grained, context-aware, and auditable access control for the complex evidence workflows characteristic of modern insurance claim handling. Existing blockchain approaches either tightly couple authorization to cryptographic schemes unsuitable for heterogeneous evidence types or enforce coarse role-based policies without addressing off-chain storage enforcement.

ClaimGuard resolves these limitations through three core mechanisms: (1) verifiable on-chain policy evaluation using smart contracts that model subjects, resources, actions, and contextual attributes; (2) a policy enforcement gateway that bridges blockchain decisions to cloud or IPFS storage via short-lived capability tokens; and (3) immutable audit trails that provide tamper-evident records of all access attempts and policy changes. This decoupling of authorization from encryption enables transparent, resource-agnostic enforcement while maintaining compatibility with existing storage infrastructure.
