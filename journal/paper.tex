...
PureChain is a private, permissioned, Ethereum-compatible blockchain employing Proof-of-Authority and Association (PoA$^2$) consensus~\cite{KIM2025100397}. Unlike proof-of-work (PoW) or proof-of-stake (PoS), PoA$^2$ relies on identity and reputation as the validator stake, making validators accountable through reputational damage and potential legal liability for misbehavior~\cite{joshi2021feasibility}. Critically, PoA$^2$ introduces a redundant standby validator mechanism that monitors active validators and automatically transfers authority if an active validator fails, enabling fault-tolerant operation without disrupting Byzantine consensus~\cite{KIM2025100397}.
...
In our evaluation, Experiment 1 (E1) uses local Hardhat (single-node, instant finality) representing an idealized PoA$^2$ network; Experiment 2 (E2) repeats measurements on Sepolia testnet (proof-of-stake) to demonstrate feasibility on public networks. The comparison reveals that ClaimGuard benefits substantially from PoA$^2$'s deterministic finality, fault-tolerance, and exceptional throughput, while remaining compatible with public blockchains for scenarios prioritizing decentralization.
