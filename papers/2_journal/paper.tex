...
\begin{table*}[!htbp]
	...
	\begin{tabular}{@{}p{2.8cm}p{2cm}p{2.2cm}p{2cm}p{1.8cm}p{1.5cm}p{1.5cm}p{2cm}@{}}
		...
		\midrule
		\textbf{ClaimGuard (This Work)}                 & \textbf{Universal} & \textbf{Agnostic} & \textbf{PureChain ABAC} & \textbf{Yes}  & \textbf{Yes}               & \textbf{Yes}      & \textbf{Quantitative \& Qualitative} \\
		\bottomrule
	\end{tabular}
...
Table~\ref{tab:security-comparison} summarizes the security trade-offs. While ClaimGuard introduces additional complexity and a reliance on consensus, it eliminates the single point of trust (the cloud provider) in traditional RBAC systems and provides immutable, verifiable policy evaluation.

\subsubsection{PureChain Network}

PureChain is a private, permissioned, Ethereum-compatible blockchain employing Proof-of-Authority and Association (PoA$^2$) consensus~\cite{ICCE2025}. Unlike proof-of-work (PoW) or proof-of-stake (PoS), PoA$^2$ relies on identity and reputation as the validator stake, making validators accountable through reputational damage and potential legal liability for misbehavior~\cite{joshi2021feasibility}. Critically, PoA$^2$ introduces a redundant standby validator mechanism that monitors active validators and automatically transfers authority if an active validator fails, enabling fault-tolerant operation without disrupting Byzantine consensus~\cite{AHAKONYE2025100354}.

PureChain achieves 7,000 transactions per second (TPS)—100× higher than standard Ethereum (30 TPS) and 1,000× higher than Bitcoin (7 TPS)—while maintaining deterministic finality within seconds~\cite{KIM2025100397}. This performance advantage stems from the small, known validator set (typically 3--7 consortium members) and elimination of proof-of-work's computational overhead. Additionally, PureChain's Smart Auto Mining Plus (SAM+) algorithm enables mining only when transactions are pending, dramatically reducing energy consumption. The EVM-compatible design allows unmodified deployment of Solidity smart contracts, eliminating porting overhead.

Consortium operation preserves data residency and regulatory compliance (CCPA, GDPR) by keeping evidence metadata within the consortium's boundaries. PoA$^2$'s identity-based validators with auditable credentials and fault-tolerant standby mechanisms naturally align with enterprise regulatory requirements, in contrast to pseudonymous PoW/PoS validators.

In our evaluation, Experiment 1 (E1) uses a local Hardhat (single-node, instant finality) to represent an idealized PoA$^2$ network; Experiment 2 (E2) repeats measurements on the Sepolia testnet (proof-of-stake) to demonstrate feasibility on public networks. The comparison reveals that ClaimGuard benefits substantially from PoA$^2$'s deterministic finality, fault-tolerance, and exceptional throughput, while remaining compatible with public blockchains for scenarios prioritizing decentralization.
...
% \label{tab:security-comparison}
% \end{table}
Overall, ClaimGuard maintains sufficient throughput for typical insurer access rates while providing stronger enforcement guarantees than RBAC.
...
\end{table}
Allow rates are similar across modes (82.6--84.1\%), indicating that ABAC policies and RBAC equivalents produce comparable outcomes on this workload. ClaimGuard's slightly lower allowance rate reflects stricter constraints (e.g., time windows, type/jurisdiction filters). No false accepts or false rejects were observed.
...
\subsubsection{Performance Trade-offs: ClaimGuard vs. Cloud-Only RBAC}
The baseline experiment (E1) demonstrates that ClaimGuard introduces a median access-check overhead of approximately 60--70\,ms compared to cloud-only RBAC (1.9\,ms at P50). This overhead arises from on-chain policy evaluation, RPC communication, and the inherent consensus latency of blockchain-backed authorization. However, this latency remains operationally acceptable for evidence access workflows, which prioritize auditability and tamper-resistance over microsecond response times. ClaimGuard sustains 100--200\, requests/second under concurrent load, sufficient for multi-stakeholder insurance claim handling where access frequency is typically measured in tens to hundreds of requests per hour per active case rather than sustained high-frequency bursts.
...
The Hybrid-Audit baseline isolates the cost of on-chain logging (136\,ms at P50) from that of full on-chain decision-making (ClaimGuard at 61\,ms). This counterintuitive result, logging alone incurs higher latency than decision-plus-logging, arises because Hybrid-Audit performs two sequential operations (local decision followed by asynchronous logging confirmation). In contrast, ClaimGuard performs a single synchronous on-chain check. This finding suggests that architectures attempting to "add blockchain for audit" to existing cloud systems may not achieve the latency benefits expected from offloading authorization.
...
Combined, these findings position ClaimGuard as the first framework to achieve verifiable, action-aware, storage-agnostic access control with demonstrated correctness, scalability, and security properties validated against real adversaries, closing the evaluation gap that plagued prior blockchain insurance systems.

\section{Conclusion}
This work extends the earlier study, ClaimGuard~\cite{eneh2025claimguard}, by providing a production-oriented validation of its blockchain-based, attribute-based access control (ABAC) architecture for multi-stakeholder auto-insurance. Beyond the initial architectural presentation (smart-contract policy layer, policy enforcement gateway, and capability-token authorization) and local-network feasibility results, the extension closes key evaluation gaps via (i) systematic baseline comparisons, (ii) deployment under real-world network conditions, (iii) security stress testing, and (iv) large-scale validation, collectively substantiating deployment readiness. ClaimGuard mitigates limitations in cloud-centric RBAC and prior blockchain access-control designs through three mechanisms: (1) verifiable on-chain policy evaluation that encodes $\langle\text{subject}, \text{resource}, \text{action}, \text{context}\rangle$ as smart-contract, enforced attributes; (2) a gateway that translates on-chain decisions into short-lived capability tokens to mediate access to cloud/IPFS-backed data; and (3) immutable, tamper-evident audit trails covering access attempts and policy evolution. By decoupling authorization from encryption, the design achieves transparent, storage-agnostic enforcement while preserving compatibility with existing storage systems.

Across E1-E4 (baseline, real-world networking, security stress, and policy-scale tests), \textsc{ClaimGuard} meets production insurance requirements. It sustains $>800$~req/s with modest overhead versus cloud-only RBAC (median $+45$-$60$~ms), keeps on-chain ABAC evaluation low-latency at $\geq 10^{3}$ policies, applies revocation immediately (no propagation delay), and achieves perfect functional correctness ($0$ false accepts/rejects). Adversarial tests show it blocks cloud-provider bypass, gateway circumvention, role spoofing, and replay attacks. More broadly, the architecture generalizes to other multi-stakeholder domains that require verifiable, fine-grained control over off-chain resources, supporting decentralized ABAC as a practical, more auditable alternative for enterprise data governance.




% This paper extended ClaimGuard~\cite{eneh2025claimguard}, a blockchain-based attribute-based access control architecture initially presented at a prior conference, with a comprehensive evaluation across four experimental dimensions that validate its readiness for production deployment in multi-stakeholder auto-insurance environments. While the conference publication introduced the core architecture comprising on-chain smart contracts, a policy enforcement gateway, and a capability token model, with an initial feasibility demonstration on local networks, this journal extension addresses critical evaluation gaps through systematic baseline comparison, real-world network deployment, security stress testing, and large-scale validation. The ClaimGuard architecture addresses fundamental limitations of both traditional cloud-centric RBAC and existing blockchain approaches through three core mechanisms: verifiable on-chain policy evaluation using smart contracts that model subjects, resources, actions, and contextual attributes; a policy enforcement gateway that bridges blockchain decisions to cloud or IPFS storage via short-lived capability tokens; and immutable audit trails providing tamper-evident records of all access attempts and policy changes. This decoupling of authorization from encryption enables transparent, resource-agnostic enforcement while maintaining compatibility with existing storage infrastructure.

% Our experimental evaluation, spanning quantitative baseline comparison (E1), real-world network conditions (E2), security stress testing (E3), and policy-scale analysis (E4), demonstrates that ClaimGuard meets the operational requirements for production insurance workflows. The system maintains throughput suitable for evidence-access workloads (exceeding 800 requests/second) while incurring modest latency overhead compared to cloud-only RBAC (median increase of 45-60ms). On-chain policy evaluation remains low-latency even under large policy counts (1000+ policies), revocation takes effect immediately without propagation delay, and functional correctness tests observed zero false accepts or false rejects across all tested scenarios. Security experiments confirmed that ClaimGuard successfully blocks unauthorized access attempts, including cloud provider bypass, gateway circumvention, role spoofing, and replay attacks, demonstrating resilience against both insider and external threats. The broader implications of this work extend beyond insurance. The ClaimGuard architecture provides a reusable pattern for any multi-stakeholder domain requiring verifiable, fine-grained access control over off-chain resources, including healthcare records, legal discovery, supply chain documentation, and regulatory compliance workflows. By demonstrating that blockchain-based ABAC can achieve practical performance while delivering stronger security and auditability guarantees than centralized alternatives, this work validates decentralized authorization as a viable approach for enterprise data governance.
