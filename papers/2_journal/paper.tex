...
\title{PACE: A Verifiable On-Chain ABAC and Off-Chain Capability Framework for Digital Evidence Governance in Multi-Stakeholder Environments\tnoteref{funding}}

\author[aff1]{Anthony Uchenna Eneh\orcidlink{0009-0000-6339-3225}}
\ead{anthony@kumoh.ac.kr}

\author[aff2]{Love Allen Chijioke Ahakonye\orcidlink{0000-0003-2840-1693}}
\ead{loveahakonye@kumoh.ac.kr}

\author[aff1]{Jae-Min Lee\orcidlink{0000-0001-6885-5185}}
\ead{ljmpaul@kumoh.ac.kr}

\author[aff1,aff3]{Dong-Seong Kim\orcidlink{0000-0002-2977-5964}\corref{cor1}}
\ead{dskim@kumoh.ac.kr}

\cortext[cor1]{Corresponding author.}
\tnotetext[funding]{This work was partly supported by the Innovative Human Resource Development for Local Intellectualization program through the IITP grant funded by the Korea government (MSIT) (IITP-2026-RS-2020-II201612, 25\%), by the Priority Research Centers Program through the NRF funded by the MEST (2018R1A6A1A03024003, 25\%), and by the MSIT, Korea, under the ITRC support program (IITP-2026-RS-2024-00438430, 25\%) and by the Basic Science Research Program through the National Research Foundation of Korea(NRF) funded by the Ministry of Education (RS-2026-25431637, 25\%).}

\address[aff1]{Department of IT Convergence Engineering, Kumoh National Institute of Technology, Gumi 39177, South Korea}
\address[aff2]{ICT Convergence Research Center, Kumoh National Institute of Technology, Gumi 39177, South Korea}
\address[aff3]{NSLab Co., Ltd., Kumoh National Institute of Technology, Gumi 39177, South Korea}

\begin{abstract}
Multi-stakeholder digital evidence workflows (healthcare, legal discovery, insurance, supply chain) require fine-grained, action-aware access controls across organizational boundaries and off-chain storage to minimize costs. Existing blockchain approaches provide tamper resistance but lack a unified, enforceable model that binds on-chain authorization to cloud or distributed backends. We generalize our prior ClaimGuard system into \textbf{PACE (Policy Anchor and Capability Enforcement)}, an Ethereum-compatible ABAC framework that anchors policy on-chain and enforces decisions via short-lived capabilities. This journal extension evaluates PACE across seven experimental dimensions: baseline comparison under load (E1), public Sepolia testnet conditions (E2), adversarial stress testing (E3), policy churn at scale up to 1000 policies and subjects (E4), PureChain consortium deployment with PoA$^2$ consensus (E5), bounded IoT/edge ingest (E6-lite), and audit query scalability across growing ledger history (E7), complemented by an ablation study isolating architectural contributions. Results show modest local-network latency overhead (60.9\,ms P50 vs.\ 1.9\,ms for RBAC) with improved authorization correctness and integrity; direct policy checks remain stable (2--3\,ms median) across 11--1000 rules, prompt denial is observed after policy revocation in churn tests, and attacks such as cloud bypass, gateway abuse, and role spoofing are blocked. PureChain consortium deployment achieves near-local latency with production-grade audit guarantees, and audit verification queries remain below 30\,ms across history windows up to 200 blocks. PACE is resource-agnostic and deployable across heterogeneous, multi-stakeholder evidence governance scenarios.
\end{abstract}

\begin{keyword}
Attribute-Based Access Control \sep Audit Trail \sep Blockchain \sep Capability-Based Authorization \sep ClaimGuard \sep Enterprise Data Governance \sep Ethereum \sep Evidence Management \sep Multi-Stakeholder Systems \sep Policy Enforcement Gateway \sep PureChain \sep Smart Contracts
\end{keyword}
...
This paper extends the PACE architecture with a comprehensive evaluation across seven experimental dimensions that address critical gaps left by the initial work. We conduct quantitative comparison against cloud-only RBAC baselines under varying concurrency and workload conditions (E1), deploy and evaluate the system on public Sepolia testnet to assess real-world network viability (E2), perform dedicated security stress testing against five attack vectors including cloud bypass and role spoofing (E3), validate scalability under policy churn with up to 1000 policies and 1000 subjects (E4), deploy PACE on a PureChain consortium network with PoA$^2$ consensus to characterize a three-tier deployment comparison spanning local, consortium, and public environments (E5), add a bounded IoT/edge deployment profile experiment (E6-lite) to evaluate attestation-aware edge evidence ingest, and benchmark audit query scalability as on-chain ledger history grows (E7). We further present an ablation study isolating the contributions of PureChain-backed audit logging and hybrid on-/off-chain policy evaluation.
...
\begin{enumerate}
	...
	\item \textbf{PureChain consortium deployment (E5):} We deploy PACE on a PureChain network with PoA$^2$ consensus and characterize a three-tier deployment comparison (local Hardhat $\rightarrow$ PureChain consortium $\rightarrow$ Sepolia public), demonstrating that consortium infrastructure achieves near-local latency with production-grade auditability.

	\item \textbf{Bounded IoT/edge deployment profile:} We add an E6-lite experiment that compares standard ingest and attestation-aware edge ingest under bounded burst workloads (simulation-based clients), reporting ingest overhead, replay blocking, invalid-attestation rejection, and burst decision latency.

	\item \textbf{Audit query scalability (E7):} We benchmark audit verification query performance as on-chain ledger history grows from 4 to 200 blocks, demonstrating that indexed event queries remain below 30\,ms and scale linearly, validating PACE's auditability claims.

	\item \textbf{Ablation study:} We isolate the contributions of PureChain-backed audit logging and hybrid on-/off-chain policy evaluation through controlled ablation experiments, demonstrating that neither component alone achieves the performance--auditability trade-off of the combined design.

	\item \textbf{Production deployment roadmap:} We provide a comprehensive analysis of experimental findings, operational recommendations, and deployment implications derived from cross-network evaluation that position PACE as a production-oriented framework for diverse multi-stakeholder evidence governance scenarios.
\end{enumerate}
...
Table~\ref{tab:threat-model} summarizes the PACE threat model. PACE addresses five threat categories common to multi-stakeholder evidence workflows. (1) Honest-but-curious cloud providers (S3/MinIO/IPFS) may attempt to read or enumerate assets; capability-based access prevents retrieval without valid, unexpired tokens issued after on-chain authorization. (2) Compromised insiders may seek cross-case access; on-chain, per-case/per-action ABAC restricts escalation~\cite{xiong2023bdim}. (3) External attackers may steal tokens or credentials; aggressive expirations and cryptographic binding to subject--action--resource tuples limit replay and misuse. (4) Man-in-the-middle adversaries may eavesdrop; PACE assumes TLS/HTTPS with standard PKI~\cite{dorri2017coordination}. (5) Malicious consortium nodes may fork, censor, or delay; we assume Byzantine fault tolerance with honest majorities (e.g., 2-of-3 or higher).

\begin{table}[!htbp]
\centering
\caption{Compact Threat Model for PACE}
\label{tab:threat-model}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}p{1.9cm}p{2.6cm}p{3cm}@{}}
\toprule
\textbf{Asset} & \textbf{Adversary Capability} & \textbf{Assumption / Mitigation} \\
\midrule
Evidence confidentiality & Cloud provider reads/enumerates stored objects & Capability-gated access; no valid token $\Rightarrow$ no retrieval \\
\addlinespace
Policy integrity & Inject stale/malicious policy into PDP cache & On-chain policy anchor; versioned updates with revocation signaling \\
\addlinespace
Decision authenticity & Forge decision provenance & Decision events tied to signer address; non-repudiable emission trail \\
\addlinespace
Audit log integrity & Compromise gateway; rewrite/delete logs & Append-only on-chain events; immutable after block confirmation \\
\addlinespace
Consensus safety & Validator collusion or Byzantine faults & PoA$^2$ permissioned validator set with bounded ($<1/3$) Byzantine tolerance \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
	\item \textbf{Consensus Failure:} If $> 1/3$ of blockchain nodes are compromised in a 3-node consortium, Byzantine consensus can fail. Large deployments should use larger consortia (5, 7, 9 nodes) or adopt PoA$^2$ with redundant standby validators and association-based trust metrics (Table~\ref{tab:consensus-comparison}).
...
Table~\ref{tab:consensus-comparison} compares consensus mechanisms relevant to blockchain-backed access control. Proof-of-Work (PoW), while battle-tested, incurs minute-scale finality and high energy costs that preclude real-time authorization~\cite{alzoubi2025pow}. Proof-of-Stake (PoS) reduces energy consumption but retains multi-second finality tied to protocol-dependent confirmation depths~\cite{shen2025consensus,shaikh2025consensus}. Practical Byzantine Fault Tolerance (PBFT) achieves deterministic finality with low latency, yet its $O(n^2)$ message complexity limits scalability beyond small validator sets~\cite{shen2025consensus}. Standard Proof-of-Authority (PoA) provides high throughput via pre-vetted validators but introduces centralization risks, including validator collusion and order manipulation~\cite{islam2025prl}. PoA$^2$ extends PoA with association-based trust metrics and redundant standby validators, achieving transaction-level deterministic finality ($<0.5$\,s), throughput exceeding 3\,K--10\,K+ TPS, and Byzantine tolerance for up to one-third of nodes~\cite{ICCE2025,AHAKONYE2025100354,ibrahim2026purechain}. These characteristics make PoA$^2$-based PureChain well-suited as a deployment target for latency-sensitive, auditable evidence governance.

\begin{table*}[!htbp]
\centering
\caption{Performance Comparison of Consensus Mechanisms for Blockchain-Backed Access Control}
\label{tab:consensus-comparison}
\scalebox{.93}{
\begin{tabular}{@{}lccccccc@{}}
\toprule
\textbf{Mechanism} & \textbf{Tx Finality} & \textbf{Throughput} & \textbf{Energy per} & \textbf{Finality} & \textbf{BFT} & \textbf{Node} & \textbf{Evidence Gov.} \\
 & \textbf{Latency} & \textbf{(TPS)} & \textbf{Tx (kWh)} & \textbf{Type} & \textbf{Tolerance} & \textbf{Scalability} & \textbf{Suitability} \\
\midrule
PoW~\cite{alzoubi2025pow} & 60--600\,s & 7--15 & $\sim$700 & Probabilistic & 50\% hash & High & Very low \\
PoS~\cite{shen2025consensus,shaikh2025consensus} & 12--384\,s & 15--30 & $\sim$0.03 & Economic & 1/3 stake & High & Low \\
PBFT~\cite{shen2025consensus} & 1--5\,s & 1\,K--3\,K & $\approx 0$ & Deterministic & 1/3 nodes & Poor ($O(n^2)$) & Moderate \\
PoA~\cite{islam2025prl} & 1--5\,s & 200--1\,K & $\approx 0$ & Near-determ. & Validator set & Moderate & Moderate \\
PoA$^2$~\cite{ICCE2025,AHAKONYE2025100354,ibrahim2026purechain} & $<$\,0.5\,s & 3\,K--10\,K+ & $\approx 0$ & Deterministic & 1/3 nodes & Good & \textbf{High} \\
\bottomrule
\end{tabular}}

\footnotesize PoW energy figures reflect Bitcoin-class mining~\cite{alzoubi2025pow}; PoS figures reflect post-Merge Ethereum~\cite{shaikh2025consensus}. PoA$^2$ achieves transaction-level finality, eliminating block-interval delays inherent in block-based protocols~\cite{AHAKONYE2025100354}.
\end{table*}

The evaluation results presented in Section~\ref{sec:implementation-and-observation} generalize to other EVM environments, provided appropriate consideration is given to consensus mechanisms, gas costs, and network latency characteristics. Public blockchains such as the Ethereum mainnet provide maximum decentralization but incur higher transaction costs and variable block times. Layer-2 solutions (Polygon, Optimism) offer cost-efficient scaling with sub-second finality. Enterprise blockchains (Besu, Quorum, PureChain) provide data residency, regulatory compliance (CCPA, GDPR), and high throughput suitable for consortium deployments where stakeholders share governance. Organizations can select deployment options based on their specific trust models, performance requirements, and regulatory constraints.
...
This section describes the PACE prototype implementation, the experimental setup, the evaluation methodology, and the observed results. We evaluate PACE by comparing blockchain-backed ABAC with policy enforcement gateway against two cloud-RBAC baselines under normal, burst, adversarial, revocation, and policy-churn workloads across three deployment tiers. Unless noted, Experiment~1 (E1) uses local Hardhat; Experiment~2 (E2) repeats key measurements on public Sepolia testnet; Experiment~5 (E5) deploys PACE on a PureChain consortium network with PoA$^2$ consensus; and Experiment~7 (E7) benchmarks audit query scalability across growing ledger history. An ablation study isolates the contributions of the hybrid on-/off-chain design and PureChain-backed audit logging.
...
To ensure fair, reproducible comparison, all three systems (PACE, RBAC-DB, Hybrid-Audit) are deployed on the same Docker host with identical hardware, network configuration, toolchain versions, and workload generator. This same-testbed methodology eliminates infrastructure confounders and ensures that measured performance differences reflect architectural choices rather than deployment artifacts. Cross-paper latency comparisons with prior work (Section~\ref{sec:implementation-and-observation}.Q) are treated as qualitative due to differences in testbed, operation definitions, and reported statistics.
...
\subsection{PureChain Consortium Deployment (E5)}
\label{sec:e5-purechain}

To bridge the gap between idealized local execution and public testnet overhead, we deploy PACE on a PureChain consortium network employing PoA$^2$ consensus (Table~\ref{tab:consensus-comparison}). The PureChain testbed uses a 3-validator consortium with transaction-level finality and the SAM+ mining algorithm. We repeat the E1 baseline workload (W1, $c{=}20$, 100 policies) and the E4 scaling workload (up to 1\,000 policies at $c{=}\{10, 50, 100, 200\}$) on PureChain to characterize a three-tier deployment comparison: Hardhat (local) $\rightarrow$ PureChain (consortium) $\rightarrow$ Sepolia (public).

\subsubsection{Baseline Authorization Latency}

Table~\ref{tab:e5-baseline} reports authorization latency percentiles under the baseline configuration ($c{=}20$, 100 policies). PureChain achieves P50 latency significantly below Sepolia while maintaining the tamper-evident audit guarantees absent from the local Hardhat environment.

\begin{table}[!htbp]
\centering
\caption{E5: Authorization Latency Percentiles (ms) --- Baseline Configuration ($c{=}20$, 100 Policies)}
\label{tab:e5-baseline}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Deployment} & \textbf{P50} & \textbf{P95} & \textbf{P99} \\
\midrule
Hardhat (local) & 60.9 & 69.6 & 78.9 \\
PureChain (consortium) & \textbf{XX.X}$^\dagger$ & \textbf{XX.X}$^\dagger$ & \textbf{XX.X}$^\dagger$ \\
Sepolia (public) & 224.7 & 246.8 & 515.3 \\
\bottomrule
\end{tabular}

\footnotesize $^\dagger$Placeholder: to be replaced with measured PureChain values.
\end{table}

\subsubsection{Scalability Under Load}

Fig.~\ref{fig:e5-scaling} presents P95 latency and throughput under increasing concurrency and policy counts on PureChain. Unlike Sepolia, where confirmation waiting dominates variance, PureChain's transaction-level finality yields predictable scaling behavior.

\begin{figure}[!htbp]
\centering
%\includegraphics[width=.46\textwidth]{./figures/e5_scaling.png}
\fbox{\parbox{0.44\textwidth}{\centering\vspace{2em}\textit{Placeholder: E5 scaling figure (P95 latency and throughput vs.\ concurrency across 1K/5K/10K policies on PureChain)}\vspace{2em}}}
\caption{E5: PureChain scalability under load. Left: P95 latency versus concurrency. Right: throughput versus concurrency across policy counts \{1K, 5K, 10K\}.}
\label{fig:e5-scaling}
\end{figure}

Under stress ($c{=}200$), PureChain P50 latency is expected to remain in the range of XX--XX\,ms across 1\,K--10\,K policies, positioning consortium deployment as the practical middle ground between local development chains (2--8\,ms) and public testnets (10\,650--27\,796\,ms). Throughput on PureChain is expected to exceed XX\,req/s at $c{=}200$, substantially above Sepolia (142--373\,req/s) and within an order of magnitude of local Hardhat throughput.

Table~\ref{tab:e5-deployment-bands} summarizes expected latency bands across deployment tiers, providing operational guidance for deployment selection.

\begin{table}[!htbp]
\centering
\caption{E5: Deployment-Tier Latency Bands}
\label{tab:e5-deployment-bands}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Deployment} & \textbf{Latency Band (ms)} & \textbf{Source} \\
\midrule
Local Hardhat & 2--79 & Measured (E1) \\
PureChain consortium & XX--XX$^\dagger$ & Measured (E5) \\
Sepolia testnet & 224--515 & Measured (E2) \\
\bottomrule
\end{tabular}
...
\subsection{Audit Query Scalability (E7)}
\label{sec:e7-audit}

A core claim of PACE is tamper-evident auditability via on-chain event logging. E7 measures how quickly an auditor can verify authorization history as the on-chain ledger grows. We define three audit query types: \textbf{Q1} (policy existence)---retrieve \texttt{PolicyUpdated} events matching a specific policy hash within a history window of $B$ blocks; \textbf{Q2} (access decision lookup)---retrieve \texttt{AccessDecision} events for a specific subject--resource pair; and \textbf{Q3} (negative verification)---confirm the \emph{absence} of access events for a given subject, requiring a full scan of the window.

\subsubsection{Query Latency vs.\ History Window}

Table~\ref{tab:e7-audit-query} reports audit query latencies as the history window $B$ increases from 4 to 200 blocks. Q1 grows approximately linearly with scanned history, while Q2 and Q3 benefit from indexed event fields and remain near-flat.

\begin{table}[!htbp]
\centering
\caption{E7: Audit Query Latency (ms) by History Window}
\label{tab:e7-audit-query}
\begin{tabular}{@{}ccccc@{}}
\toprule
$B$ \textbf{(blocks)} & \textbf{Q1 (policy)} & \textbf{Q2 (access)} & \textbf{Q3 (negative)} & \textbf{Logs scanned} \\
\midrule
4 & XX.X$^\dagger$ & XX.X$^\dagger$ & XX.X$^\dagger$ & 2 \\
10 & XX.X$^\dagger$ & XX.X$^\dagger$ & XX.X$^\dagger$ & 5 \\
25 & XX.X$^\dagger$ & XX.X$^\dagger$ & XX.X$^\dagger$ & 12 \\
50 & XX.X$^\dagger$ & XX.X$^\dagger$ & XX.X$^\dagger$ & 25 \\
100 & XX.X$^\dagger$ & XX.X$^\dagger$ & XX.X$^\dagger$ & 50 \\
200 & XX.X$^\dagger$ & XX.X$^\dagger$ & XX.X$^\dagger$ & 100 \\
\bottomrule
\end{tabular}

\footnotesize $^\dagger$Placeholder: to be replaced with measured values.
\end{table}

A linear regression on Q1 yields slope $\approx$ XX.X\,ms/block with $R^2 \approx$ XX.X, supporting approximate linear scaling with scanned history depth. Across all window sizes, Q2 and Q3 remain below XX\,ms, confirming that indexed event queries scale efficiently for access-decision lookup and negative verification.

\subsubsection{Same-Testbed SUT vs.\ Baseline Comparison}

To isolate the benefit of on-chain indexed event logging, we compare PACE audit queries against a baseline (B1-style) workflow that retrieves raw transaction receipts and filters client-side under identical hardware and RPC conditions.

\begin{table}[!htbp]
\centering
\caption{E7: Same-Testbed PACE vs.\ B1-Style Query Workflow (Mean Latency in ms)}
\label{tab:e7-sut-vs-b1}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Query} & \textbf{PACE} & \textbf{B1-style} & \textbf{B1/PACE Ratio} \\
\midrule
Q1 (policy existence) & XX.X$^\dagger$ & XX.X$^\dagger$ & XX.X$\times$ \\
Q2 (access existence) & XX.X$^\dagger$ & XX.X$^\dagger$ & XX.X$\times$ \\
Q3 (negative query) & XX.X$^\dagger$ & XX.X$^\dagger$ & XX.X$\times$ \\
\bottomrule
\end{tabular}
...
PACE's indexed event queries are expected to outperform B1-style receipt scanning for access-decision lookup (Q2) and negative verification (Q3) by a significant margin, while Q1 performance depends on event density within the scanned window. These results validate the auditability claims central to PACE's design.
...
\subsubsection{Ablation Study: Architectural Contributions}

We conduct an ablation analysis isolating two design factors: (i) the use of PureChain with PoA$^2$ consensus versus a public blockchain for audit logging, and (ii) the hybrid on-/off-chain policy evaluation versus fully on-chain ABAC. Two ablation variants are evaluated under identical baseline conditions ($c{=}20$, 100 policies) on the same testbed.

\paragraph{Ablation~1 (Centralized PDP + Public Blockchain Audit):} Replacing PureChain with Ethereum Sepolia for audit logging increases authorization latency by approximately $10^2\times$ (P50: 60.9\,ms $\rightarrow$ XX.X$^\dagger$\,ms) and reduces throughput to XX.X$^\dagger$\,req/s, due to multi-second block times and network contention. Audit query latency remains moderate (XX.X$^\dagger$\,ms), but end-to-end performance becomes infeasible for real-time evidence workflows.

\paragraph{Ablation~2 (Fully On-Chain ABAC on PureChain):} Moving policy evaluation fully on-chain increases latency by $>$\,XX$\times$ (P50: XX.X$^\dagger$\,ms) and limits throughput (XX.X$^\dagger$\,req/s), reflecting EVM computation and contract invocation overhead for complex attribute evaluation within a single transaction. Audit query latency remains comparable, indicating logging efficiency is preserved.

\noindent This demonstrates that neither component alone is sufficient. Public chains violate latency constraints for interactive evidence access, while fully on-chain evaluation degrades scalability at enterprise policy counts. The combined hybrid design---off-chain evaluation with PureChain-backed audit logging---achieves the optimal trade-off: low-latency authorization with verifiable, tamper-evident auditability.

\noindent\footnotesize $^\dagger$Placeholder: to be replaced with measured ablation values.
\normalsize
...
\section{Conclusion}
\label{con}
We presented PACE, an Ethereum-compatible, action-aware attribute-based access control framework for multi-stakeholder digital evidence governance, and validated it through production-oriented experiments spanning baseline comparisons (E1), public network latency (E2), adversarial stress tests (E3), large-scale policy churn (E4), PureChain consortium deployment with PoA$^2$ consensus (E5), a bounded IoT/edge ingest profile (E6-lite), audit query scalability (E7), and an ablation study isolating architectural contributions. Results show low-latency, high-throughput authorization with prompt policy-layer revocation in the measured access path and strong resistance to gateway and token attacks, while preserving storage-agnostic enforcement and compatibility with existing IAM via the Policy Enforcement Gateway. PureChain consortium deployment achieves near-local latency with tamper-evident audit guarantees, and audit verification queries remain efficient across growing ledger history. PACE’s resource-agnostic policy model enables cross-domain deployment through configuration rather than code changes, making it suitable for healthcare, legal discovery, insurance, and supply-chain scenarios that require auditable, multi-organization access control. Limitations and future directions are summarized below.
