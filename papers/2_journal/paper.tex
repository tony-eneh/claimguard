...
While PACE provides verifiable access control and protection against honest-but-curious cloud providers, the security analysis must address residual risks, attack vectors, and distributed-system trade-offs. We summarize mitigations, limitations, and operational assumptions below.
...
We presented PACE, an Ethereum-compatible, action-aware attribute-based access control framework for multi-stakeholder digital evidence governance, and validated it through production-oriented experiments spanning baseline comparison, public-network latency, adversarial stress tests, and large-scale policy churn. Results show low-latency, high-throughput authorization with immediate revocation and strong resistance to gateway and token attacks, while preserving storage-agnostic enforcement and compatibility with existing IAM via the Policy Enforcement Gateway.

PACE’s resource-agnostic policy model enables cross-domain deployment through configuration rather than code changes, making it suitable for healthcare, legal discovery, and supply-chain scenarios that require auditable, multi-organization access control. Limitations and future directions are summarized below.
