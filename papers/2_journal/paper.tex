...
\section{Conclusion}
\label{con}
We presented PACE, an Ethereum-compatible, action-aware attribute-based access control framework for multi-stakeholder digital evidence governance, and validated it through production-oriented experiments spanning baseline comparisons, public network latency, adversarial stress tests, large-scale policy churn, and a bounded IoT/edge ingest profile. Results show low-latency, high-throughput authorization with prompt policy-layer revocation in the measured access path and strong resistance to gateway and token attacks, while preserving storage-agnostic enforcement and compatibility with existing IAM via the Policy Enforcement Gateway. PACE’s resource-agnostic policy model enables cross-domain deployment through configuration rather than code changes, making it suitable for healthcare, legal discovery, insurance, and supply-chain scenarios that require auditable, multi-organization access control. Limitations and future directions are summarized below.
