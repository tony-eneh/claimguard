...
\thanks{Manuscript received April 2026; revised April 2026; accepted April 2026.}
...
\subsection{Architectural Delta from ClaimGuard}

The core ClaimGuard architecture (on-chain ABAC evaluation, stateless gateway mediation, and short-lived capability enforcement) remains intact in PACE, and this continuity is intentional for controlled extension and reproducibility. Relative to the conference version, the primary delta in this revision is a broadened systems profile rather than a replacement of the trust boundary or contract topology: stronger baseline coverage (RBAC and hybrid audit), public-network validation, adversarial stress testing, large-scale policy churn analysis, and a bounded IoT/edge ingest profile.

For architectural evolution beyond the preserved core, we define three concrete extension paths used to structure implementation and validation in this revision cycle: (i) an emergency revocation overlay to reduce practical revoke-lag windows at the gateway boundary, (ii) an asynchronous/batched audit plane to decouple decision latency from audit persistence strategy, and (iii) indexed policy-evaluation pathways to reduce policy-scan overhead at larger rule counts. This decomposition separates currently implemented and measured behavior from planned deltas, while keeping the manuscript's claims aligned with available artifacts.
...
\begin{figure}[!htbp]
	...
	\caption{E1 throughput comparison across RBAC, Hybrid-Audit, and PACE under W1 normal load.}
	\label{fig:throughput}
\end{figure}
Fig.~\ref{fig:throughput} compares throughput across the three E1 systems as concurrency increases from 10 to 200 users. RBAC remains in the 6.2k--7.0k\,req/s range because authorization is fully local. PACE rises from 169.7\,req/s at 10 users to 575.9\,req/s at 200 users, while Hybrid-Audit rises from 71.9\,req/s to 340.7\,req/s over the same range. This shows that PACE materially outperforms the audit-coupled baseline while remaining slower than pure local RBAC due to synchronous blockchain-backed decisions.
Overall, PACE maintains sufficient throughput for typical insurer access rates while providing stronger enforcement guarantees than RBAC and substantially better throughput than Hybrid-Audit in the same E1 workload.
...
\begin{table}[ht]
	...
	\begin{tabular}{lccc}
		...
		\midrule
		Role Spoofing\footnotemark & 12         & 50.0               & PARTIAL         \\
...
\end{table}
\footnotetext{The partial result for role spoofing reflects that malformed (non-checksummed) Ethereum addresses bypass client-side validation but are rejected at the contract level; no valid authorization bypass occurred across any test vector.}
E3 confirms the core invariant: unauthorized evidence access is blocked at the file server (capability validation) and the gateway rejects malformed/abusive requests without granting access. The role-spoofing \emph{partial} result reflects that malformed (non-checksummed) Ethereum addresses bypass client-side validation but are rejected at the contract level; no valid authorization bypass occurred across any test vector. Expired/tampered token rejection is also observed in E1.
...
The baseline experiment (E1) demonstrates that PACE introduces a median access-check overhead of approximately 60--70\,ms compared to cloud-only RBAC (1.9\,ms at P50). This overhead arises from on-chain policy evaluation, RPC communication, and the inherent consensus latency of blockchain-backed authorization. However, this latency remains operationally acceptable for evidence access workflows, which prioritize auditability and tamper-resistance over microsecond response times. In the three-way E1 throughput comparison, PACE sustains roughly 170--576\,req/s depending on concurrency, which remains sufficient for multi-stakeholder insurance claim handling where access frequency is typically measured in tens to hundreds of requests per hour per active case rather than sustained high-frequency bursts.

Policy operations (add, update, revoke) cost approximately 21.9\,ms and 128k gas on average. While higher than database updates, policy changes are infrequent administrative operations compared to access checks. The gas cost translates to under \$0.01 USD on many consortium or Layer-2 networks, making policy management economically feasible for production deployments. In the churn tests, policy-layer denial was observed promptly after revocation, but residual storage access still remains bounded by the lifetime of any previously issued short-lived capability.
...
E3 security stress experiments confirm that PACE successfully blocks unauthorized access attempts across five attack vectors: cloud provider bypass (80\% block rate), gateway abuse (100\%), role spoofing (50\%, with caveats), token replay, and revocation evasion. The partial result for role spoofing reflects that malformed (non-checksummed) Ethereum addresses bypass client-side validation but are rejected at the contract level; no valid authorization bypass occurred across any test vector. Well-formed spoofed requests with invalid credentials were consistently rejected. These results validate the core security hypothesis: capability-based access with gateway-enforced signatures prevents cloud operators from enumerating or retrieving evidence via direct storage API calls. Unlike encryption-only approaches (CP-ABE, proxy re-encryption) that couple authorization to key distribution, ClaimGuard's token-based model supports prompt policy-layer revocation while bounding residual storage access through short capability lifetimes instead of key re-issuance or ciphertext re-encryption. The immutable on-chain audit trail provides forensic evidence for post-incident analysis, deterring insider abuse through transparency and non-repudiation. Compared to traditional cloud-RBAC, ClaimGuard eliminates the single point of trust represented by centralized IAM systems and database-backed permission tables. While introducing dependency on blockchain consensus liveness, this trade-off is favorable for regulated domains where policy integrity and audit verifiability outweigh the operational complexity of running consortium nodes.

\subsubsection{Real-World Viability: Public Testnet Performance (E2)}

Deploying PACE on Sepolia testnet reveals the performance impact of public blockchain infrastructure. In E2, access latency increases to 224.7\,ms (P50) from 16.0\,ms on local Hardhat, with P90 at 246.8\,ms and P99 near 515.3\,ms, primarily due to variability in public RPC providers rather than gateway or contract compute. Policy update confirmation times increase from roughly 20\,ms locally to 24.8\,s on average on Sepolia, with worst-case observations near 89.4\,s, dominated by public-network confirmation and RPC variability. Despite these increases, Sepolia throughput (142--373\,req/s) remains operationally viable for evidence access, demonstrating that PACE can function on public infrastructure without requiring private consortium deployment. For production use cases requiring lower latency, organizations can deploy on consortium networks (Hyperledger Besu, Quorum, private Ethereum PoA) or Layer-2 solutions (Polygon, Optimism) to achieve local-network performance while retaining public settlement for cross-organizational disputes. The E2 findings highlight a key architectural decision: policy updates should be treated as asynchronous administrative operations initiated by governance workflows, not synchronous user-facing actions. Access checks, in contrast, require synchronous on-chain evaluation but benefit from caching strategies (e.g., short-lived subject attribute caches) to reduce repeated RPC calls for the same user within a session.

\subsubsection{Scalability and Policy Churn (E4)}

E4 tests PACE's behavior under large policy counts (100--1000 policies) and high subject populations (500--1000 subjects). Median access latency remains stable (increasing from 61\,ms to only 71\,ms at 1000 policies), indicating that, for the measured workload, policy growth does not dominate the end-to-end decision path. In the current implementation, \texttt{checkAccess} still performs a linear scan with first-match allow semantics; the observed stability therefore reflects workload characteristics and the fact that gateway, RPC, and storage-path overhead dominate the request path rather than a proven indexed evaluator. Gas costs for policy operations scale with rule complexity but remain under 200k gas even for complex multi-condition rules. These results suggest PACE can handle enterprise-scale policy sets (thousands of rules across hundreds of resources) within the tested envelope, while a true indexed evaluator remains future engineering work rather than a reported implementation result.
...
The experimental results demonstrate that PACE's performance characteristics, access latency (60--70\,ms on local networks), workload-dependent local throughput (170--576\,req/s in the E1 three-way baseline comparison and 569--999\,req/s in the E2 local comparison setting), policy update overhead (21--22\,ms), and prompt denial after measured revocation events are determined primarily by consensus, RPC, gateway, and storage-path costs rather than by domain-specific policy vocabulary. Scaling behavior with respect to policy count (1000 policies, latency increase of only 10\,ms) reflects the tested workload and the fact that end-to-end latency is dominated by infrastructure effects even though the present contract logic still uses a linear policy scan. Security guarantees against unauthorized access, replay attacks, and insider privilege escalation are enforced by immutable smart contract logic and cryptographic capability token validation, both domain-agnostic mechanisms.
...
\subsubsection{IoT/Edge Deployment Profile (E6-lite)}

To include IoT relevance without diluting the paper's primary scope, we evaluated a bounded edge-ingest profile (E6-lite) in which simulated edge producers submit evidence through the same policy-gateway path, with and without attestation-aware ingest checks. The experiment is intentionally narrow: 200 simulated edge producers and 4000 total ingest requests, comparing standard ingest against attestation-aware ingest under identical burst settings.

E6-lite reports four required metrics. Attestation-aware ingest adds a mean ingest overhead of 5.028\,ms; replay blocking improves to 99.67\%; invalid-attestation rejection reaches 99.58\%; and mean decision latency under burst for the attestation-aware path is 34.904\,ms. These results support the claim that IoT/edge evidence sources can be incorporated as a deployment profile of PACE with bounded overhead while preserving the same policy-evaluation and capability-enforcement structure.

The scope remains constrained. E6-lite uses simulation-based edge clients and a deterministic attestation-checking interface rather than hardware-rooted remote attestation, and therefore should be interpreted as an architectural feasibility signal rather than a complete industrial IoT benchmark.
...
ClaimGuard outperforms several prior blockchain and cloud access control solutions in the insurance and healthcare sectors while exposing practical trade-offs. Liu et al.~\cite{liu2021auto} report encryption costs of 47.22 ms for encryption and 102.58 ms for re-encryption in their auto insurance scheme; ClaimGuard achieves 61 ms P50 access latency locally and 244 ms on the public testnet, demonstrating decentralized policy evaluation without key distribution overhead. Unlike Rana et al.~\cite{rana2022healthcare}, who offer only conceptual validation, ClaimGuard provides quantitative performance across multiple workloads: 170--576 req/s in the E1 three-way baseline comparison, 569--999 req/s in the E2 local comparison setting, and 142--373 req/s on the public testnet. While Sun et al.~\cite{sun2020emr} focus on security analysis without latency benchmarks, ClaimGuard undergoes stress testing against five attack vectors with block rates of 50--100\%, providing empirical threat-mitigation evidence. Tekale \& Rahul~\cite{tekale2023claims} propose smart contract-based claims settlement but offer no security metrics; ClaimGuard reports zero false accepts over 10,000 checks, prompt denial after measured revocation events in churn tests, and immutable audit trails. Most prior work uses coarse RBAC models (Raikwar et al.~\cite{raikwar2018insurance}, Tekale \& Rahul) or expensive cryptography (Liu et al., Sun et al.), leading to scalability issues. ClaimGuard maintains stable measured latency (61--71 ms) across 100--1000 policies in the tested workload, and E4 shows policy-operation costs under 200k gas, supporting feasibility of on-chain attribute-based authorization in regulated workflows.


\section{Conclusion}
\label{con}
We presented PACE, an Ethereum-compatible, action-aware attribute-based access control framework for multi-stakeholder digital evidence governance, and validated it through production-oriented experiments spanning baseline comparisons, public network latency, adversarial stress tests, large-scale policy churn, and a bounded IoT/edge ingest profile. Across these experiments, PACE maintained operationally acceptable authorization latency while preserving tamper-resistant policy state, auditable decision traces, and storage-agnostic enforcement through the Policy Enforcement Gateway. The results show that blockchain-backed ABAC can be implemented without sacrificing the responsiveness required for practical evidence-handling workflows, especially when deployed on local, consortium, or Layer-2 infrastructure. This practical significance is substantial: to the best of our knowledge, PACE is the first framework to provide quantitative evidence that blockchain-backed ABAC is operationally viable for multi-stakeholder evidence governance. The framework’s resource-agnostic policy model enables cross-domain deployment through configuration rather than code changes, making it suitable for healthcare, legal discovery, insurance, and supply-chain scenarios that require auditable, multi-organization access control. The E3 and E4 findings further show that the architecture can preserve authorization correctness under adversarial pressure while remaining stable across larger policy sets and churn-heavy administrative conditions. Beyond performance, PACE contributes a verifiable governance model in which policy intent, access decisions, and revocation events are externally auditable instead of being hidden inside a single cloud administrator’s trust boundary. All core artifacts used in this study, including smart contracts, gateway code, and evaluation scripts, are available as open-source resources to support reproducibility, scrutiny, and follow-on research. This makes the framework useful not only as a deployment blueprint but also as a reusable experimental baseline for future work on verifiable access control. More broadly, the architecture has implications for regulatory compliance in environments shaped by frameworks such as the EU AI Act and the Digital Operational Resilience Act (DORA), where traceable decision governance, controlled data access, and demonstrable accountability are becoming mandatory design concerns. PACE therefore offers a concrete systems path toward compliant, multi-party governance of high-stakes digital evidence. Limitations and future directions are summarized below.

\subsection{Limitations and Future Work}
The evaluation tested insurance workflows, and broader deployments in healthcare, legal discovery, and supply chains are still needed for stronger generalizability. Our E6-lite IoT/edge profile is intentionally bounded: it uses simulation-based edge clients and does not yet include hardware-rooted remote attestation. The policy model also lacks privacy-preserving mechanisms, and tools for authoring, onboarding, and audits are still in development. Multi-jurisdictional governance needs stronger temporal constraints and stronger support for obligations. Future work will focus on full cross-domain validation, stronger attestation and edge trust chains, privacy-preserving mechanisms, hybrid evaluation, and pilot deployments, prioritizing decentralized identity standards and formal smart contract verification for better interoperability and correctness.
